...
public class Utils {
	...
	public static ArrayList<Brackets> findBrackatedExpression(String docText)
		...
		while (matcher.find()) {
			...
			System.out.println("******Bracket\t" + matched + "\t" + start
					+ "\t" + end);
	...
	public static ArrayList<QuestionAnswerSet> getQuestionAnswerSet(JCas jCas) {
		FSList qaSetList = Utils.getTestDocumentFromCAS(jCas).getQaList();
		ArrayList<QuestionAnswerSet> qaList = new ArrayList<QuestionAnswerSet>();
		int i = 0;
		while (true) {
			QuestionAnswerSet qaSet = null;
			try {
				qaSet = (QuestionAnswerSet) qaSetList.getNthElement(i);
			} catch (Exception e) {
				break;
			}
			qaList.add(qaSet);
	...
	public static ArrayList<Token> getTokenlistFromQuestion(Question question) {
		...
		FSList queList = question.getTokenList();
		ArrayList<Token> tokenList = new ArrayList<Token>();
		int i = 0;
		while (true) {
			...
			Token token = null;
			try {
				token = (Token) queList.getNthElement(i);
			} catch (Exception e) {
				break;
			}
			tokenList.add(token);
	...
	public static ArrayList<Token> getTokenlistFromAnwser(Answer answer) {

		FSList ansList = answer.getTokenList();
		ArrayList<Token> tokenList = new ArrayList<Token>();
		int i = 0;
		while (true) {
			...
			Token token = null;
			try {
				token = (Token) ansList.getNthElement(i);
			} catch (Exception e) {
				break;
			}
			tokenList.add(token);
	...
	public static ArrayList<Answer> getAnswerListFromQASet(
			QuestionAnswerSet qaSet) {

		FSList ansList = qaSet.getAnswerList();
		...
		while (true) {
			...
			Answer answer = null;
			try {
				answer = (Answer) ansList.getNthElement(i);
			} catch (Exception e) {
				break;
			}
