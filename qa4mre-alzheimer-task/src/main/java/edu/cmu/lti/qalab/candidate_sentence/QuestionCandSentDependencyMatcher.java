...
public class QuestionCandSentDependencyMatcher {
	...
	public QuestionCandSentDependencyMatcher(String path,String depTreePath) throws Exception{
	...
	public double getScore(Sentence target,Sentence candidate) throws IOException{
		...
		for(Dependency t:targtList){
			...
			String d2 = maxC.getRelation()+"("+maxC.getGovernor().getText() +","+maxC.getDependent().getText()+")";
